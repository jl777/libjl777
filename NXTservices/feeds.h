...
uint64_t ensure_orderbook(int32_t *polarityp,char *base,char *rel)
{
    uint64_t baseid,relid,obookid;
    struct raw_orders *raw;
    baseid = get_orderbook_assetid(base);
    relid = get_orderbook_assetid(rel);
    if ( baseid == 0 || relid == 0 )
        return(0);
...
int32_t parse_json_quotes(double *quotes,cJSON *array,int32_t maxdepth,char *pricefield,char *volfield)
{
    cJSON *item;
    int32_t i,n;
    ...
    for (i=0; i<n; i++)
    {
        ...
        {
            quotes[(i<<1)] = get_API_float(cJSON_GetObjectItem(item,pricefield));
        ...
        {
            quotes[(i<<1)] = get_API_float(cJSON_GetArrayItem(item,0));
            quotes[(i<<1) + 1] = get_API_float(cJSON_GetArrayItem(item,1));
        }
        else printf("unexpected case in parse_json_quotes\n");
    }
    return(n);
}

struct orderbook_tx **parse_json_orderbook(int32_t *nump,int32_t type,uint64_t nxt64bits,uint64_t obookid,int32_t polarity,int32_t maxdepth,cJSON *json,char *resultfield,char *bidfield,char *askfield,char *pricefield,char *volfield)
{
    ...
    if ( resultfield == 0 || (obj= cJSON_GetObjectItem(json,resultfield)) != 0 )
    {
        ...
        if ( (bidobj= cJSON_GetObjectItem(obj,bidfield)) != 0 && is_cJSON_Array(bidobj) != 0 )
            numbids = parse_json_quotes(bids,bidobj,maxdepth,pricefield,volfield);
        if ( (askobj= cJSON_GetObjectItem(obj,askfield)) != 0 && is_cJSON_Array(askobj) != 0 )
            numasks = parse_json_quotes(asks,askobj,maxdepth,pricefield,volfield);
        orders = conv_quotes(nump,type,nxt64bits,obookid,polarity,bids,numbids,asks,numasks);
        free(bids);
        free(asks);
    }
    return(orders);
...
/*
int32_t parse_cryptsy(char *market,int32_t maxdepth) // "BTCD" is "256", "NXT" is "159"
{
    int32_t n = 0;
    char url[512],*jsonstr;
    return(n);
        {"success":1,"return":
 {"BTCD":{"marketid":"256","label":"BTCD\/BTC","primaryname":"BitcoinDark","primarycode":"BTCD","secondaryname":"BitCoin","secondarycode":"BTC",
 "sellorders":[{"price":"0.00125136","quantity":"58.00000000","total":"0.07257888"},{"price":"0.00125145","quantity":"124.00000000","total":"0.15517980"},{"price":"0.00232001","quantity":"0.20196476","total":"0.00046856"},{"price":"0.00234200","quantity":"9.99900000","total":"0.02341766"},{"price":"0.00234501","quantity":"0.61448754","total":"0.00144098"},{"price":"0.00235000","quantity":"0.42858160","total":"0.00100717"},{"price":"0.00237000","quantity":"0.04493178","total":"0.00010649"},{"price":"0.00238200","quantity":"8.32105115","total":"0.01982074"},{"price":"0.00240000","quantity":"160.51623573","total":"0.38523897"},{"price":"0.00241000","quantity":"8.83272570","total":"0.02128687"},{"price":"0.00241820","quantity":"1.00000000","total":"0.00241820"}],
     
     "buyorders":[{"price":"0.00122489","quantity":"138.00000000","total":"0.16903482"},{"price":"0.00122488","quantity":"107.35245519","total":"0.13149388"},{"price":"0.00122484","quantity":"1.65965039","total":"0.00203281"},{"price":"0.00122480","quantity":"80.00000000","total":"0.09798400"},{"price":"0.00104147","quantity":"0.32005994","total":"0.00033333"},{"price":"0.00103946","quantity":"26.93966899","total":"0.02800271"}]}}}
}*/


struct orderbook_tx **parse_cryptsy(int32_t *nump,char *base,char *rel,int32_t maxdepth) // "BTC-BTCD"
{
    ...
    uint64_t obookid;
    int32_t polarity,i,m,n = 0;
    cJSON *json,*obj,*marketjson;
    char url[512],*jsonstr,market[128];
    struct orderbook_tx **orders = 0;
    if ( strcmp(rel,"BTC") != 0 )
    {
    ...
    for (i=0; i<m; i++)
    {
        ...
        if ( strcmp(base,get_cJSON_fieldname(obj)) == 0 )
        {
            sprintf(market,"%d",(int32_t)_get_cJSON_int(obj));
            break;
        }
    }
    free(marketjson);
    ...
    obookid = ensure_orderbook(&polarity,base,rel);
    if ( obookid == 0 )
    {
        printf("parse_cryptsy: null obookid\n");
        return(0);
    }
    sprintf(url,"http://pubapi.cryptsy.com/api.php?method=singleorderdata&marketid=%s",market);
    jsonstr = _issue_curl(0,"cryptsy",url);
    if ( jsonstr != 0 )
    {
        if ( (json = cJSON_Parse(jsonstr)) != 0 )
        {
            ...
            {
                orders = parse_json_orderbook(&n,ORDERBOOK_FEED,CRYPTSY_FEEDID,obookid,polarity,maxdepth,obj,base,"buyorders","sellorders","price","quantity");
            }
            free_json(json);
        }
        free(jsonstr);
...
struct orderbook_tx **parse_bittrex(int32_t *nump,char *base,char *rel,int32_t maxdepth) // "BTC-BTCD"
{
    uint64_t obookid;
    int32_t polarity,n = 0;
    cJSON *json,*obj;
    char url[512],*jsonstr,market[128];
    struct orderbook_tx **orders = 0;
    obookid = ensure_orderbook(&polarity,base,rel);
    ...
    jsonstr = _issue_curl(0,"bittrex",url);
    ...
    if ( jsonstr != 0 )
    {
        if ( (json = cJSON_Parse(jsonstr)) != 0 )
        {
            if ( (obj= cJSON_GetObjectItem(json,"success")) != 0 && is_cJSON_True(obj) != 0 )
                orders = parse_json_orderbook(&n,ORDERBOOK_FEED,BITTREX_FEEDID,obookid,polarity,maxdepth,obj,"result","buy","sell","Rate","Quantity");
            free_json(json);
        }
        free(jsonstr);
    }
...
struct orderbook_tx **parse_poloniex(int32_t *nump,char *base,char *rel,int32_t maxdepth) // "BTC-BTCD"
{
    uint64_t obookid;
    int32_t polarity,n = 0;
    cJSON *json;
    ...
    jsonstr = _issue_curl(0,"poloniex",url);
    ...
    if ( jsonstr != 0 )
    {
        if ( (json = cJSON_Parse(jsonstr)) != 0 )
        {
            orders = parse_json_orderbook(&n,ORDERBOOK_FEED,POLONIEX_FEEDID,obookid,polarity,maxdepth,json,0,"bids","asks",0,0);
            free_json(json);
        }
        free(jsonstr);
    }
...
struct orderbook_tx **parse_bter(int32_t *nump,char *base,char *rel,int32_t maxdepth)
{
    uint64_t obookid;
    int32_t polarity,n = 0;
    cJSON *json,*obj;
    char url[512],resultstr[512],*jsonstr;
    struct orderbook_tx **orders = 0;
    obookid = ensure_orderbook(&polarity,base,rel);
    if ( obookid == 0 )
        return(0);
    sprintf(url,"http://data.bter.com/api/1/depth/%s_%s",base,rel);
    jsonstr = _issue_curl(0,"bter",url);
    ...
    if ( jsonstr != 0 )
    {
        if ( (json = cJSON_Parse(jsonstr)) != 0 )
        {
            if ( (obj= cJSON_GetObjectItem(json,"result")) != 0 )
            {
                ...
                {
                    orders = parse_json_orderbook(&n,ORDERBOOK_FEED,BTER_FEEDID,obookid,polarity,maxdepth,obj,0,"bids","asks",0,0);
                }
            }
            free_json(json);
        }
...
struct orderbook_tx **parse_mintpal(int32_t *nump,char *base,char *rel,int32_t maxdepth)
{
    uint64_t obookid;
    int32_t polarity,n = 0;
    cJSON *json,*bidobj,*askobj;
    char url[512],*buystr,*sellstr;
    struct orderbook_tx **orders = 0;
    obookid = ensure_orderbook(&polarity,base,rel);
    if ( obookid == 0 )
        return(0);
    sprintf(url,"https://api.mintpal.com/v1/market/orders/%s/%s/BUY",base,rel);
    buystr = _issue_curl(0,"mintpal",url);
    sprintf(url,"https://api.mintpal.com/v1/market/orders/%s/%s/SELL",base,rel);
    sellstr = _issue_curl(0,"mintpal",url);
//{"count":3,"type":"BUY","orders":[{"price":"0.00000003","amount":"137066327.96066495","total":"4.11198982"},{"price":"0.00000002","amount":"293181381.39291047","total":"5.86362760"},{"price":"0.00000001","amount":"493836943.18472463","total":"4.93836939"}]}
    if ( buystr != 0 && sellstr != 0 )
    {
        ...
        orders = parse_json_orderbook(&n,ORDERBOOK_FEED,MINTPAL_FEEDID,obookid,polarity,maxdepth,json,0,"bids","asks","price","amount");
        free_json(json);
    }
    if ( buystr != 0 )
        free(buystr);
