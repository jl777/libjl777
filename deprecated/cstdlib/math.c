...
extern uint64_t PTL_placebid(char *base,char *rel,double price,double volume);
extern uint64_t PTL_placeask(char *base,char *rel,double price,double volume);
extern int32_t PTL_makeoffer(char *base,char *rel,double price,double volume,uint64_t nxtaddr);

void LibPlacebid(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->UnsignedLongInteger = PTL_placebid(Param[0]->Val->Pointer,Param[1]->Val->Pointer,Param[2]->Val->FP,Param[3]->Val->FP);
}
void LibPlaceask(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->UnsignedLongInteger = PTL_placeask(Param[0]->Val->Pointer,Param[1]->Val->Pointer,Param[2]->Val->FP,Param[3]->Val->FP);
}
void LibMakeoffer(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->Integer = PTL_makeoffer(Param[0]->Val->Pointer,Param[1]->Val->Pointer,Param[2]->Val->FP,Param[3]->Val->FP,Param[4]->Val->UnsignedLongInteger);
}
...
struct LibraryFunction MathFunctions[] =
{
    ...
    //{ LibSendmessage,   "int sendmessage(uint64_t nxtaddr,char *msg);" },
    { LibPlacebid,   "unsigned long placebid(char *base,char *rel,double price,double volume);" },
    { LibPlaceask,   "unsigned long placeask(char *base,char *rel,double price,double volume);" },
    { LibMakeoffer,   "int makeoffer(char *base,char *rel,double price,double volume,unsigned long nxtaddr);" },
    { NULL,             NULL }
};
