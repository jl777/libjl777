...
void StdioFprintfWord(int32_t longflag,StdOutStream *Stream, const char *Format,uint64_t Value)
{
    char longfmt[5];
    if ( longflag != 0 )
    {
            ...
            if (ShowType != &VoidType)
            {
                ...
                else
                {
                    ...
                    if (ShowType == &IntType)
                    {
                        ...
                        {
                            StdioFprintfWord(longflag,&SOStream, OneFormatBuf, ExpressionCoerceUnsignedInteger(ThisArg));
                        }
                        else
                            StdioOutPuts("XXX", &SOStream);
                    }
...
{
    ReturnValue->Val->Integer = (int)fread(Param[0]->Val->Pointer, Param[1]->Val->Integer, Param[2]->Val->Integer, Param[3]->Val->Pointer);
}

void StdioFwrite(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) 
{
    ReturnValue->Val->Integer = (int)fwrite(Param[0]->Val->Pointer, Param[1]->Val->Integer, Param[2]->Val->Integer, Param[3]->Val->Pointer);
}
