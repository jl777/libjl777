...
void TypeInit()
{
    ...
    TypeAddBaseType(&IntType, TypeInt, sizeof(int32_t), IntAlignBytes);
    TypeAddBaseType(&ShortType, TypeShort, sizeof(int16_t), (char *)&sa.y - &sa.x);
    TypeAddBaseType(&CharType, TypeChar, sizeof(unsigned char), (char *)&ca.y - &ca.x);
    TypeAddBaseType(&LongType, TypeLong, sizeof(int64_t), (char *)&la.y - &la.x);
    TypeAddBaseType(&UnsignedIntType, TypeUnsignedInt, sizeof(uint32_t), IntAlignBytes);
    TypeAddBaseType(&UnsignedShortType, TypeUnsignedShort, sizeof(uint16_t), (char *)&sa.y - &sa.x);
    TypeAddBaseType(&UnsignedLongType, TypeUnsignedLong, sizeof(uint64_t), (char *)&la.y - &la.x);
