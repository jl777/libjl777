...
enum LexToken LexGetWord(struct LexState *Lexer, struct Value *Value)
{
    ...
    Value->Typ = NULL;
    Value->Val->Identifier = TableStrRegister2(StartPos, (int)(Lexer->Pos - StartPos));
...
enum LexToken LexGetStringConstant(struct LexState *Lexer, struct Value *Value, char EndChar)
{
    ...
    EscBuf = HeapAllocStack((int)(EndPos - StartPos));
    if (EscBuf == NULL)
    ...
    RegString = TableStrRegister2(EscBuf, (int)(EscBufPos - EscBuf));
    HeapPopStack(EscBuf, (int)(EndPos - StartPos));
...
enum LexToken LexGetRawToken(struct ParseState *Parser, struct Value **Value, int IncPos)
{
    ...
    { 
        ...
        { 
            ...
            char *LineBuffer = 0;//[LINEBUFFER_MAX];
            void *LineTokens;
            int LineBytes;
            struct TokenLine *LineNode;
            if ( LineBuffer == 0 )
                LineBuffer = malloc(LINEBUFFER_MAX);
            ...
            { 
                ...
                LineTokens = LexAnalyse(StrEmpty, &LineBuffer[0], (int)strlen(LineBuffer), &LineBytes);
...
void *LexCopyTokens(struct ParseState *StartParser, struct ParseState *EndParser)
{
    ...
    { 
        ...
        { 
            ...
            MemSize = (int)(&InteractiveCurrentLine->Tokens[InteractiveCurrentLine->NumBytes-TOKEN_DATA_OFFSET] - Pos);
            ...
            CopySize = (int)(&InteractiveCurrentLine->Tokens[InteractiveCurrentLine->NumBytes-TOKEN_DATA_OFFSET] - Pos);
