...
void process_lotto(cJSON **jsonp,int32_t iter,uint64_t buyerbits,int64_t assetoshis,int32_t blockid,uint64_t txid)
{
    int32_t i,n,num;
    cJSON *item = 0;
    char numstr[64],rsaddr[64];
    uint32_t numtickets,numplayers,firstblock = 0;
    uint64_t buyer = 0,seed = 0;
    int64_t total = 0;
        ...
        for (i=0; i<n; i++)
        {
            ...
            if ( iter < 0 || buyerbits == buyer )
            {
                ...
                if ( total > 0 && total >= MIN_REQUIRED )
                {
                    ...
                    //    cJSON_AddItemToObject(item,"tickets",gen_lottotickets_json(seed,num));
                    numtickets += num;
                    numplayers++;
                }
            }
    ...
    if ( iter < 0 )
    {
        printf("Total tickets %d players.%d ave %.1f\n",numtickets,numplayers,(double)numtickets/numplayers);
        return;
    }
    if ( i == n )
    {
...
char *update_lotto_transactions(char *refNXTaddr,char *assetidstr)
{
    char cmd[1024],*jsonstr;
    int32_t createdflag,blockid,iter,i,n = 0;
    cJSON *item,*json,*array,*jsonarg;
    uint64_t refbits,sellerbits,buyerbits,qnt,txid;
    struct NXT_asset *ap;
    struct coin_info *cp;
                ...
                if ( (array= cJSON_GetObjectItem(json,(iter & 1) ? "transfers" : "trades")) != 0 && is_cJSON_Array(array) != 0 && (n= cJSON_GetArraySize(array)) > 0 )
                {
                    for (i=0; i<n; i++)
                    {
                        ...
                        if ( ((iter&2) == 0 && sellerbits == refbits) || ((iter&2) != 0 && buyerbits == refbits) )
                            process_lotto(&jsonarg,iter,(iter&2) == 0 ? buyerbits : sellerbits,qnt * (1 - (iter&2)),blockid,txid);
                    }
                }
                free_json(json);
            }
    ...
    process_lotto(&jsonarg,-1,0,0,0,0);
