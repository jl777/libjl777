...
int64_t process_NXTtransaction(char *sender,char *receiver,cJSON *item,char *refNXTaddr,char *assetid,int32_t syncflag,char *coinstr)
{
...
int32_t init_NXT_transactions(int32_t txtype,char *refNXTaddr,char *coinstr)
{
        ...
        if ( (json= cJSON_Parse(jsonstr)) != 0 )
        {
            if ( (array= cJSON_GetObjectItem(json,"transactions")) != 0 && is_cJSON_Array(array) != 0 && (n= cJSON_GetArraySize(array)) > 0 )
            {
                for (i=0; i<n; i++)
                {
                    ...
                    item = cJSON_GetArrayItem(array,i);
                    val = process_NXTtransaction(sender,receiver,item,refNXTaddr,assetid,0,coinstr);
                }
            }
            free_json(json);
        }
...
int64_t process_msigdeposits(cJSON **transferjsonp,int32_t forceflag,struct coin_info *cp,struct address_entry *entry,uint64_t nxt64bits,struct NXT_asset *ap,char *msigaddr)
{
    ...
    if ( j == ap->num )
    {
        ...
        if ( strcmp(msigaddr,coinaddr) == 0 && txidstr[0] != 0 && value >= (cp->NXTfee_equiv * MIN_DEPOSIT_FACTOR) )
        {
            ...
            if ( j == ap->num )
            {
                ...
                if ( forceflag > 1 || ready_to_xferassets() > 0 )
                {
...
uint64_t process_msigaddr(uint64_t *unspentp,cJSON **transferjsonp,int32_t forceflag,struct NXT_asset *ap,char *refassetid,char *NXTaddr,struct coin_info *cp,char *msigaddr)
{
    ...
    {
        init_NXT_transactions(2,NXTaddr,cp->name);
        if ( Debuglevel > 2 )
...
char *MGWdeposits(char *specialNXT,int32_t rescan,int32_t transferassets,char *coin,char *assetstr,char *NXT0,char *NXT1,char *NXT2,char *ip0,char *ip1,char *ip2)
{
    ...
    char retbuf[1024],*gateways[3],*ipaddrs[3],*transferstr = 0;
    struct coin_info *cp;
    cJSON *transferjson = 0;
    uint64_t circulation,unspent,total = 0;
    ...
    if ( rescan != 0 )
    {
        init_NXT_transactions(1,specialNXT,coin); // side effect updates MULTISIG_DATA from AM
        for (i=0; i<numgateways; i++)
            init_NXT_transactions(1,gateways[i],coin);
    ...
    sprintf(retbuf,"{\"circulation\":\"%.8f\",\"unspent\":\"%.8f\",\"pendingdeposits\":\"%.8f\",\"transfers\":%s}\n",dstr(circulation),dstr(unspent),dstr(total),transferstr!=0?transferstr:"[]");
    if ( transferstr != 0 )
        free(transferstr);
    return(clonestr(retbuf));
}
