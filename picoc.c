...
double picoc(int argc,char **argv,char *codestr)
{
    ...
    PicocIncludeAllSystemHeaders();
    if ( PicocPlatformSetExitPoint() != 0 )
    {
        printf("picoc error doing setjmp\n");
        PicocCleanup();
        return PicocExitValue;
    }
    PicocParse("picoc codestr",codestr,(int)strlen(codestr),TRUE,FALSE,TRUE);
    PicocCallMain(argc,argv);
    PicocCleanup();
    //printf("exit value %d %.15f\n",(int)PicocExitValue,PicocExitValue);
    return PicocExitValue;
}
