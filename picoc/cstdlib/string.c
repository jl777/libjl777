...
{
    ReturnValue->Val->Integer = (int)strspn(Param[0]->Val->Pointer, Param[1]->Val->Pointer);
}

void StringStrcspn(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ReturnValue->Val->Integer = (int)strcspn(Param[0]->Val->Pointer, Param[1]->Val->Pointer);
}
...
{
    ReturnValue->Val->Integer = (int)strxfrm(Param[0]->Val->Pointer, Param[1]->Val->Pointer, Param[2]->Val->Integer);
}
