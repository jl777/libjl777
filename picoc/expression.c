    ...
    while (StackTop != NULL)
    {
        ...
        { 
            ...
            switch (StackTop->Val->Typ->Base)
            {
                ...
                case TypeUnsignedShort: printf("%u:unsigned short", StackTop->Val->Val->UnsignedShortInteger); break;
                case TypeUnsignedInt: printf("%u:unsigned int", StackTop->Val->Val->UnsignedInteger); break;
                case TypeUnsignedLong: printf("%lu:unsigned long", StackTop->Val->Val->UnsignedLongInteger); break;
...
uint64_t ExpressionCoerceInteger(struct Value *Val)
{
    //printf("exprcoer.(%llx)\n",(long long)Val->Val->UnsignedLongInteger);
    switch (Val->Typ->Base)
    {
        case TypeInt:             return (uint64_t)Val->Val->Integer;
        case TypeChar:            return (uint64_t)Val->Val->Character;
        case TypeShort:           return (uint64_t)Val->Val->ShortInteger;
        case TypeLong:            return (uint64_t)Val->Val->LongInteger;
        case TypeUnsignedInt:     return (uint64_t)Val->Val->UnsignedInteger;
        case TypeUnsignedShort:   return (uint64_t)Val->Val->UnsignedShortInteger;
        case TypeUnsignedLong:    return (uint64_t)Val->Val->UnsignedLongInteger;
        case TypePointer:         return (uint64_t)Val->Val->Pointer;
#ifndef NO_FP
        case TypeFP:              return (uint64_t)Val->Val->FP;
#endif
        default:                  return 0;
    }
}
...
double ExpressionCoerceFP(struct Value *Val)
{
    ...
    switch (Val->Typ->Base)
    {
        ...
        case TypeLong:            IntVal = (int64_t)Val->Val->LongInteger; return (double)IntVal;
        ...
        case TypeUnsignedLong:    UnsignedVal = (uint64_t)Val->Val->UnsignedLongInteger; return (double)UnsignedVal;
...
int64_t ExpressionAssignInt(struct ParseState *Parser, struct Value *DestValue, int64_t FromInt, int After)
{
    int64_t Result;
    ...
    {
        case TypeInt:           DestValue->Val->Integer = (int32_t)FromInt; break;
        case TypeShort:         DestValue->Val->ShortInteger = (int16_t)FromInt; break;
        case TypeChar:          DestValue->Val->Character = (unsigned char)FromInt; break;
        case TypeLong:          DestValue->Val->LongInteger = (int64_t)FromInt; break;
        case TypeUnsignedInt:   DestValue->Val->UnsignedInteger = (uint32_t)FromInt; break;
        case TypeUnsignedShort: DestValue->Val->UnsignedShortInteger = (uint16_t)FromInt; break;
        case TypeUnsignedLong:  DestValue->Val->UnsignedLongInteger = (uint64_t)FromInt; break;
        default: break;
    }
    //printf("from %llx to %llx\n",(long long)FromInt,(long long)DestValue->Val->UnsignedLongInteger);
    return Result;
}
...
void ExpressionAssign(struct ParseState *Parser, struct Value *DestValue, struct Value *SourceValue, int Force, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    switch (DestValue->Typ->Base)
    {
        case TypeInt:           DestValue->Val->Integer = (int32_t)ExpressionCoerceInteger(SourceValue); break;
        ...
        case TypeUnsignedInt:   DestValue->Val->UnsignedInteger = (uint32_t)ExpressionCoerceUnsignedInteger(SourceValue); break;
            ...
            { 
                ...
                { 
                    ...
                    { 
                        ...
                        {
                            int64_t LHSInt = ExpressionCoerceInteger(StackTop->Val);
