...
struct crypto777_node *crypto777_node(char *url,char *email,bits384 mypassword,char *type)
{
    ...
    if ( (nn= calloc(1,sizeof(*nn))) != 0 )
    {
        ...
        calc_SaM(&nn->emailhash,(uint8_t *)email,(int32_t)strlen(email),0,0,13);
        hit = calc_SaM(&nn->nxtpass,(uint8_t *)email,(int32_t)strlen(email),mypassword.bytes,(int32_t)sizeof(nn->mypassword),13);
        ...
        nn->mypubkey = SaM_chain(email,mypassword,nn->pubkeychainid,0,nn->revealed,NUMROUNDS_CHAIN);
