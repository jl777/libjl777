...
uint64_t simple_wallet::get_rawbalance()
{
    return(m_wallet->balance());
}

uint64_t simple_wallet::get_confbalance()
{
    return(m_wallet->unlocked_balance());
}

void simple_wallet::load_blocks()
{
    //size_t fetched_blocks = 0;
    if ( m_daemon_host.empty() != 0 )
        m_daemon_host = "localhost";
    if ( m_daemon_port == 0 )
        m_daemon_port = RPC_DEFAULT_PORT;
...
std::string simple_wallet::get_address(currency::account_public_address &addr)
{
    std::string str;
    str = m_wallet->get_account().get_public_address_str();
    get_transfer_address(str,addr);
    return(str);
}
...
int main(int argc, char* argv[])
{
  ...
  if ( argc == 1 )
  {
    char *_argv[3];
    _argv[0] = argv[0];
    _argv[1] = (char *)"--wallet-file=wallet.bin";
    _argv[2] = 0;
    argv = _argv; argc = 2;
  }
  //TRY_ENTRY();
